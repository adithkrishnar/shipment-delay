...
from app.schemas.upload import ImportResponse, MapRequest, MapResponse, UploadResponse, ValidateRequest, ValidationIssue, ValidationReport
...
async def upload_dataset(company_id: int=Form(...), dataset_type: str=Form(...), file: UploadFile=File(...), db: Session=Depends(get_db)):
    ...
    upload_record = DatasetUpload(company_id=company_id, dataset_type=dataset_type, original_filename=file.filename or 'upload', stored_path='', status='uploaded')
    ...
    logger.info('Uploaded dataset id=%s company=%s type=%s rows=%s file=%s', upload_record.id, company_id, dataset_type, len(df), file.filename)
    return UploadResponse(upload_id=upload_record.id, company_id=company_id, dataset_type=dataset_type, original_filename=upload_record.original_filename, row_count=len(df), columns=list(df.columns), preview_rows=preview_rows(df), suggested_mapping=suggested_mapping, status=upload_record.status)

@router.post('/data/validate', response_model=ValidationReport)
def validate_upload(payload: ValidateRequest, db: Session=Depends(get_db), *, current_user: User=Depends(get_current_user)):
    ...
    upload.validation_report_json = json.dumps({'errors': report['errors'], 'warnings': report['warnings'], 'missing_required_fields': report['missing_required_fields']})
    db.commit()
    return ValidationReport(upload_id=upload.id, data_quality_score=report['data_quality_score'], row_count=report['row_count'], valid_row_count=report['valid_row_count'], errors=[ValidationIssue(**e) for e in report['errors']], warnings=[ValidationIssue(**w) for w in report['warnings']], missing_required_fields=report['missing_required_fields'])

@router.post('/data/map', response_model=MapResponse)
def map_upload(payload: MapRequest, db: Session=Depends(get_db), *, current_user: User=Depends(get_current_user)):
    upload = _require_upload(db, payload.upload_id)
    ...
    db.commit()
    return MapResponse(upload_id=upload.id, column_mapping=payload.column_mapping, unmapped_required_fields=missing, status=upload.status)

@router.post('/data/import', response_model=ImportResponse)
def import_upload(company_id: int, upload_id: int, db: Session=Depends(get_db), *, current_user: User=Depends(get_current_user)):
    ...
    if missing:
        raise HTTPException(status_code=400, detail=f'Cannot import: required fields not mapped: {missing}')
    df = read_tabular_file(Path(upload.stored_path))
    ...
    db.commit()
    logger.info('Imported dataset upload_id=%s company=%s type=%s imported=%s skipped=%s', upload_id, company_id, upload.dataset_type, imported, skipped)
    ...
    return ImportResponse(upload_id=upload_id, company_id=company_id, dataset_type=upload.dataset_type, imported_row_count=imported, skipped_row_count=skipped, status=upload.status)

@router.get('/data/uploads/{company_id}')
def list_uploads(company_id: int, db: Session=Depends(get_db), *, current_user: User=Depends(get_current_user)):
    if not current_user.is_superuser and current_user.company_id != company_id:
        raise HTTPException(403, "Not authorized to access this company's data")
    _require_company(db, company_id)
    uploads = db.query(DatasetUpload).filter(DatasetUpload.company_id == company_id).order_by(DatasetUpload.uploaded_at.desc()).all()
    return [{'upload_id': u.id, 'dataset_type': u.dataset_type, 'original_filename': u.original_filename, 'status': u.status, 'row_count': u.row_count, 'data_quality_score': u.data_quality_score, 'uploaded_at': u.uploaded_at.isoformat(), 'imported_at': u.imported_at.isoformat() if u.imported_at else None} for u in uploads]