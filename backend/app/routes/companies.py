...
def list_companies(db: Session=Depends(get_db), *, current_user: User=Depends(get_current_user)):
    ...
    for c in companies:
        ...
        date_bounds = db.query(func.min(Sale.date), func.max(Sale.date)).filter(Sale.company_id == c.id).first()
        out.append(CompanySummaryOut(id=c.id, name=c.name, industry=c.industry, is_demo=c.is_demo, default_currency=c.default_currency, created_at=c.created_at, product_count=product_count, supplier_count=supplier_count, sales_record_count=sales_count, shipment_count=shipment_count, date_range_start=str(date_bounds[0]) if date_bounds and date_bounds[0] else None, date_range_end=str(date_bounds[1]) if date_bounds and date_bounds[1] else None))
    return out
...
def create_company(payload: CompanyCreate, db: Session=Depends(get_db), *, current_user: User=Depends(get_current_user)):
    ...
    company = Company(name=payload.name, industry=payload.industry, default_currency=payload.default_currency, notes=payload.notes, is_demo=0)
    db.add(company)
    db.commit()
    db.refresh(company)
